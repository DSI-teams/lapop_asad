...
# Testing effect of lengthen function
*Note for Jesse* Here I checked what the long versions of `argentina_unique_unfactor` and `argentina_16_unfactor` looked like and what the effect of `union_all()` is.
...
# test %>% 
#   count(person_id) %>% 
#   arrange(desc(n)) #This results in n = 508 for all IDs
# 
# test2 %>% 
#   count(person_id) %>% 
#   arrange() #This results in n = 208 which makes sense because this is only one wave
# 
# combined_test %>% 
#   count(person_id) %>% 
#   arrange(desc(n)) #This just shows that the two datasets are stacked on top of each other
```
...
# Testing effect of lengthen function
*Note for Jesse* Here I checked what the long versions of `argentina_unique_factor` and `argentina_16_factor` looked like and what the effect of `union_all()` is.

```{r}
# test3 <- lengthen_factor(argentina_unique_factor)
# test4<- lengthen_16factor(argentina_16_factor)
# combined_test2 <- union_all(test3, test4)
# 
# test3 %>% 
#   count(person_id_factor) %>% 
#   arrange(desc(n)) #This results in n = 508 for all IDs - same as above
# 
# test4 %>% 
#   count(person_id_factor) %>% 
#   arrange() #This results in n = 208 which makes sense because this is only one wave - same as above
# 
# a <- combined_test2 %>% 
#   count(person_id_factor) %>% 
#   arrange(desc(n)) #This just shows that the two datasets are stacked on top of each other - same as above
```
Cleaning up the combined_test*
```{r}
rm("combined_test")
rm("combined_test2")
```
...
```{r}
total_clean <- list(argentina_clean, belize_clean, brazil_clean, chile_clean, bolivia_clean, costa_clean, ecuador_clean, elsalvador_clean, guatemala_clean, guyana_clean, honduras_clean, mexico_clean, nicaragua_clean, panama_clean, paraguay_clean, peru_clean, suriname_clean, uruguay_clean, venezuela_clean)
...
#We would not expect the person_id_factor to be unique. It is repeated for every question a person answers, so no need to run this assert
#assert(combined_clean, is_uniq, person_id_factor)

```

## Data Checks Below (no need to run)
...
We need to make sure person_id_factor x question is unique. Start with a check for a single country. 

```{r}
argentina_clean %>% 
  count(person_id_factor, question) %>% 
  arrange(desc(n)) 
```


It is unique. This should be unique for all countries in the combined, as well. 
